...
export default function MailList({ onOpenSidebar, labels, mails, activeLabelName }: Props) {
  ...
  const filteredMailsIds = useMemo(() => mails.allIds.filter(id => (
      ...
      {!isEmpty ? (
        <Scrollbar>
          <Box sx={{ minWidth: { md: 800 } }}>
            {filteredMailsIds.map((mailId) => (
              !!mails.byId[mailId].labels?.filter(label => (
                activeLabelName && 
                ((label?.name === activeLabelName && label.custom) || 
                (activeLabelName === 'sent' && mails.byId[mailId].senderPartyId === (user?.profiles && user.profiles[0].partyId)) || 
                (activeLabelName === 'inbox' && mails.byId[mailId].senderPartyId !== (user?.profiles && user.profiles[0].partyId))))).length || 
                (mails.byId[mailId].starred && activeLabelName === 'Starred'))
            &&
            (
              <MailItem
                key={mailId}
