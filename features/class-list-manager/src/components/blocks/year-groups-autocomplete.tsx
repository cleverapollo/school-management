...
import { useYearGroups, ReturnTypeFromUseYearGroups } from '@tyro/api';
...
export const YearGroupsAutocomplete = ({
  ...
  const options = useMemo(
    () =>
      years
        ?.filter((year) => year.yearGroupId > 0)
        .sort((prev, next) => prev.name.localeCompare(next.name) ?? 0) ?? [],
    [years]
  );
  ...
  return (
    <Autocomplete
      ...
      value={value}
      options={options}
      isOptionEqualToValue={(option, { yearGroupId }) =>
        option.yearGroupId === yearGroupId
      }
